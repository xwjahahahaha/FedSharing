...
func (n *discoveryNotifee) HandlePeerFound(pi peer.AddrInfo) {
	if pi.ID != n.h.ID() {
		logger.Logger.Infof("discovered new peer %s", pi.ID.Pretty())
		err := n.h.Connect(context.Background(), pi)
		if err != nil {
			logger.Logger.Warnf("error connecting to peer %s: %s", pi.ID.Pretty(), err)
		}
		logger.Logger.Infof("success connect new peer %s !", pi.ID.Pretty())
		connectPeerChan <- pi.ID
	}
}
